...
# Fixes:
#   1. Credentials → st.secrets (ไม่ต้อง upload JSON)
#   2. Thread-safe — ไม่มี global ACTIVE_BANK / FORCE_YEAR
#   3. session_state — ผลลัพธ์ไม่หายเมื่อ interact กับ UI
#   4. ลบ redundant build_final_parsed_stm_from_check call
#   5. Temp files cleanup ด้วย shutil.rmtree
# ============================================================
...
# ============================================================
# 1) Constants — shared ทุก session, ไม่ใช่ per-user state
# ============================================================
...
def get_bank_config(bank_name):
    # FIX: รับ bank_name โดยตรง ไม่อ่าน global
    return BANK_CONFIGS.get(bank_name, BANK_CONFIGS["DEFAULT"])


# ============================================================
# 4) Google Vision Client — จาก st.secrets, cached ทุก session
# ============================================================

@st.cache_resource
def create_vision_client():
    """
    FIX: ใช้ st.secrets แทน file upload + os.environ
    ...
    try:
        ...
        st.error(
            "❌ ไม่พบ `gcp_service_account` ใน Streamlit Secrets\n\n"
            "กรุณาเพิ่มใน **Settings → Secrets** (Streamlit Cloud) "
        ...
        st.error(f"❌ สร้าง Vision Client ไม่ได้: {e}")
        st.stop()


# ============================================================
# 5) Image Preprocessing — ใช้ tmp_dir ที่ส่งเข้ามา ไม่ใช้ gettempdir()
# ============================================================
...
def preprocess_image_variants(input_path, tmp_dir):
    ...
    def _save(img_obj, suffix):
        p = os.path.join(tmp_dir, f"pre_{base_name}_{suffix}.png")
        img_obj.save(p)
        return p
    ...
    variants.append(("2x_contrast_sharp", _save(img1, "2x_contrast_sharp")))
    ...
    variants.append(("3x_gray_autocontrast", _save(img2, "3x_gray_autocontrast")))
    ...
    variants.append(("3x_unsharp", _save(img3, "3x_unsharp")))
    ...
    variants.append(("3x_bw_threshold_175", _save(img4, "3x_bw_threshold_175")))
    ...
    variants.append(("3x_bw_threshold_145", _save(img5, "3x_bw_threshold_145")))
    ...
    variants.append(("3x_denoise_sharp", _save(img6, "3x_denoise_sharp")))
...
def pick_best_money_candidate(candidates):
    ...
    candidates = sorted(
        candidates,
        key=lambda x: (score_money_candidate(x.get("text", ""), x.get("value")), x.get("x_center", 0)),
        reverse=True,
    )
...
def amount_balance_match(prev_balance, amount, balance, tolerance=BALANCE_TOLERANCE):
    if any(v is None or pd.isna(v) for v in [prev_balance, amount, balance]):
        return None
...
def try_healing_amount(prev_balance, read_amount, balance, tolerance=0.05):
    if any(v is None or pd.isna(v) for v in [prev_balance, balance]):
        return None, None
...
# ============================================================
# 7) Date Helpers — force_year ส่งเป็น parameter ไม่ใช่ global
# ============================================================
...
def normalize_date(s, force_year):
    # FIX: รับ force_year เป็น parameter
    s = fix_ocr_date_token(s)
...
def extract_date_from_line(line_text, force_year):
    ...
    for token in str(line_text).split():
        ...
        if is_date_token(token_fixed):
            return normalize_date(token_fixed, force_year)
    return None
...
# ============================================================
# 8) Text / Transaction Helpers — bank_name ส่งเป็น parameter เสมอ
# ============================================================
        ...
        if is_money_token(text) or extract_money_values_loose(text):
            ...
            if value is not None:
                money_items.append({
                    ...
                    "confidence": row.get("word_confidence"),
                })
    return money_items


def has_opening_balance_text(text, bank_name):
    # FIX: bank_name บังคับส่งเข้ามา ไม่มี fallback ไป global
    ...
    for _, row in g.sort_values("x_center").iterrows():
        ...
        if amount_x_min <= x < amount_x_max:
            ...
            for value in extract_money_values_loose(text):
                ...
                money_texts.append(str(text))
    return {
        "amount": pick_best_money_candidate(amount_candidates),
        "balance": pick_best_money_candidate(balance_candidates),
        "money_texts": money_texts,
    }


def parse_scb_line(g, active_bank, force_year, page_no=None):
    # FIX: รับ active_bank, force_year เป็น parameter
    g = g.sort_values("x_center").copy()
    raw_line_text = join_words(g)
    if is_noise_line(raw_line_text):
        return None
    date = extract_date_from_line(raw_line_text, force_year)
    time = extract_time_from_line(raw_line_text)
    line_confidence = min_confidence = None
    if "word_confidence" in g.columns:
        line_confidence = float(g["word_confidence"].mean())
        min_confidence = float(g["word_confidence"].min())
    if has_opening_balance_text(raw_line_text, active_bank):
        ...
        return {
            ...
            "money_tokens": " | ".join(money_texts), "money_count": len(money_texts),
            "raw_line_text": raw_line_text, "y_center": g["y_center"].mean(),
            "parsed_date": pd.NaT, "is_opening_balance": True,
    ...
    return {
        ...
        "amount": money_info.get("amount"), "balance": money_info.get("balance"),
        "money_tokens": " | ".join(money_info.get("money_texts", [])),
        "money_count": len(money_info.get("money_texts", [])),
        "raw_line_text": raw_line_text, "y_center": g["y_center"].mean(),
        "parsed_date": parse_date_for_sort(date), "is_opening_balance": False,
        "line_confidence": line_confidence, "min_confidence": min_confidence,
    }


# ============================================================
# 10) Balance Chain Sorting
# ============================================================
...
def advanced_build_chain_with_healing(work_df, start_index, tolerance=0.05):
    ...
    while not candidates.empty:
        found_index = found_match = None
        is_healed_match = False
        healed_info = {}
        for idx, row in candidates.iterrows():
            match = amount_balance_match(prev_balance, row.get("amount"), row.get("balance"), tolerance)
...
def smart_order_by_balance_chain(df, active_bank):
    # FIX: รับ active_bank เป็น parameter
    if df.empty:
        return df
    if active_bank == "SCB":
        sort_cols = []
    ...
    best_ordered = best_remaining = None
    best_score = -999999
    for option in possible_starts:
        test_df = option["df"].copy().reset_index(drop=True)
        ordered_df, remaining_df, match_count = advanced_build_chain_with_healing(
            test_df, option["start_index"], tolerance=BALANCE_TOLERANCE
        )
            ...
            for paragraph_idx, paragraph in enumerate(block.paragraphs):
                for word_idx, word in enumerate(paragraph.words):
                    ...
                    rows.append({
                        ...
                        "block_idx": block_idx, "paragraph_idx": paragraph_idx,
                        "word_idx": word_idx, "text": word_text,
                        "word_confidence": word.confidence,
                        "x_min": word_box["x_min"], "y_min": word_box["y_min"],
                        "x_max": word_box["x_max"], "y_max": word_box["y_max"],
                        "x_center": word_box["x_center"], "y_center": word_box["y_center"],
                        "width": word_box["width"], "height": word_box["height"],
                    })
...
def parse_simple_stm(words_df, active_bank, force_year):
    ...
    for (file_name, line_id), g in line_words_df.groupby(["file_name", "line_id"]):
        ...
        if active_bank == "SCB":
            parsed_line = parse_scb_line(g, active_bank, force_year, page_no=page_no)
        ...
        date = extract_date_from_line(raw_line_text, force_year)
        time = extract_time_from_line(raw_line_text)
        if not date and not has_opening_balance_text(raw_line_text, active_bank):
            continue
        money_items = extract_money_from_line_words(g)
        ...
        line_rows.append({
            "file_name": file_name, "line_id": int(line_id), "page_no": page_no,
            "date": date, "time": time, "code": None,
            "amount": amount, "balance": balance,
            "money_tokens": " | ".join(money_texts), "money_count": len(money_values),
            "raw_line_text": raw_line_text, "y_center": g["y_center"].mean(),
            "parsed_date": parse_date_for_sort(date),
            "is_opening_balance": has_opening_balance_text(raw_line_text, active_bank),
            "line_confidence": line_confidence, "min_confidence": min_confidence,
        })
...
def score_parsed_result(parsed_df):
    ...
    return (
        len(parsed_df) * 100
        + parsed_df["money_count"].fillna(0).sum() * 20
        + parsed_df["date"].notna().sum() * 50
        + conf_score
    )


def ocr_best_variant_for_image(image_path, client, active_bank, force_year, tmp_dir, status_box=None):
    # FIX: รับ active_bank, force_year, tmp_dir เป็น parameter
    ...
    for v_name, v_path in variants:
        try:
            ...
            if best_score is None or score > best_score:
                best_score = score
                best = {
                    "variant_name": v_name, "variant_path": v_path,
                    "full_text": full_text, "word_df": word_df, "score": score,
                }
        except Exception as e:
...
def is_valid_number(x):
    if x is None:
        return False
    try:
        return not pd.isna(x) and float(x) is not None
    except Exception:
        return False
...
def is_opening_row(row):
    ...
    # NOTE: has_opening_balance_text ต้องการ bank_name แต่ถูกเรียกจาก check_df ที่ไม่มี context
    # ใช้ code/balance_check แทนเพื่อ safety
    return (
        "ยอดยกมา" in raw_line_text.lower()
        or "brought forward" in raw_line_text.upper()
        or "ยอด ยก มา" in raw_line_text
        or code == "OPENING"
...
def add_debit_credit_and_check(ordered_df, active_bank):
    ...
    check_cols = [
        ...
        "suggested_amount", "suggested_type", "expected_balance", "diff",
        "line_confidence", "min_confidence", "money_tokens", "raw_line_text",
        "order_method", "balance_check",
    ]
    ...
    for _, row in ordered_df.iterrows():
        ...
        balance_check = ""
        low_conf = (
            (line_confidence is not None and not pd.isna(line_confidence) and line_confidence < LOW_CONFIDENCE_THRESHOLD)
            or (min_confidence is not None and not pd.isna(min_confidence) and min_confidence < LOW_CONFIDENCE_THRESHOLD)
        )
        if has_opening_balance_text(raw_line_text, active_bank) or code == "OPENING":
                ...
                if healed_amt is not None:
                    ...
                    if suggested_amount is not None and suggested_type in ["debit", "credit"]:
                        expected_balance = round(
                            prev_balance - suggested_amount if suggested_type == "debit"
                            else prev_balance + suggested_amount, 2
                        )
                        diff = round(balance_val - expected_balance, 2)
        ...
        check_rows.append({
            ...
            "expected_balance": expected_balance, "diff": diff,
            "line_confidence": line_confidence, "min_confidence": min_confidence,
            "money_tokens": money_tokens, "raw_line_text": raw_line_text,
    ...
    # FIX: ไม่ต้อง build final_df ที่นี่ — ให้ caller เรียก build_final_parsed_stm_from_check เอง
    return check_df
...
def build_excel_bytes(final_df, check_df, selected_bank, tmp_dir):
    ...
    ocr_review_df = check_df[
        check_df["balance_check"].astype(str).str.contains(
            "REVIEW|BROKEN|UNKNOWN|NO_PREV|MISSING|SUGGEST|HEALED", regex=True, na=False
        )
...
# ============================================================
# 14) Streamlit UI — Sidebar (ไม่มี JSON uploader แล้ว)
# ============================================================
...
with st.sidebar:
    ...
    selected_bank = st.selectbox(
        "เลือกธนาคาร",
        ["KBANK", "KRUNGSRI", "BBL", "SCB", "DEFAULT"],
        index=0,
        help="เลือกธนาคารเอง ระบบจะไม่ Auto Detect",
    )
    selected_year = st.number_input(
        "ปีของ Statement",
        min_value=2000, max_value=2100,
        value=datetime.now().year, step=1,
        help="ถ้า Statement ไม่มีปีในรายการ ระบบจะใช้ปีนี้",
    )
    st.divider()
    st.caption("🔐 Google Vision Credentials โหลดจาก Streamlit Secrets อัตโนมัติ")
    st.caption("ตั้งค่าที่ Settings → Secrets (Streamlit Cloud) หรือ `.streamlit/secrets.toml`")

uploaded_images = st.file_uploader(
    "อัปโหลดรูป Statement",
    type=["jpg", "jpeg", "png"],
...
if process_btn:
    ...
    # ดึง Vision client (cached — สร้างครั้งเดียว)
    client = create_vision_client()

    # FIX: ไม่แตะ global — ใช้ local variables เท่านั้น
    active_bank = selected_bank
    force_year = int(selected_year)

    st.info(f"ธนาคารที่เลือก: **{active_bank}** | ปี Statement: **{force_year}**")

    # FIX: tmp_dir เดียวสำหรับทุกอย่าง → cleanup ตอนจบ
    ...
    try:
        ...
        with st.spinner("กำลัง OCR และเลือก image variant ที่ดีที่สุด..."):
            for i, img_file in enumerate(uploaded_images):
                suffix = os.path.splitext(img_file.name)[1].lower() or ".png"
                img_path = os.path.join(tmp_dir, f"input_{i}{suffix}")
                with open(img_path, "wb") as f:
                    f.write(img_file.read())
                with log_area:
                    st.write(f"กำลังประมวลผล: {img_file.name}")
                best = ocr_best_variant_for_image(
                    img_path, client, active_bank, force_year, tmp_dir, status_box=log_area
                )
        ...
        with st.spinner("กำลังแยกรายการ Statement และตรวจสอบ balance chain..."):
            parsed_df = parse_simple_stm(words_raw_df, active_bank, force_year)
            dedup_df = deduplicate_rows(parsed_df)
            ordered_df = smart_order_by_balance_chain(dedup_df, active_bank)
            # FIX: add_debit_credit_and_check คืน check_df เท่านั้น
            check_df = add_debit_credit_and_check(ordered_df, active_bank)
            # FIX: เรียก build_final_parsed_stm_from_check แค่ครั้งเดียว ที่นี่ที่เดียว
            final_df = build_final_parsed_stm_from_check(check_df)

        excel_bytes, summary_df, ocr_review_df = build_excel_bytes(
            final_df, check_df, active_bank, tmp_dir
        )

        # FIX: เก็บผลลัพธ์ใน session_state — ไม่หายเมื่อ user interact กับ UI
        ...
        st.session_state["force_year"] = force_year

    except Exception as e:
        st.exception(e)
    finally:
        # FIX: cleanup temp files ทุกกรณี
        shutil.rmtree(tmp_dir, ignore_errors=True)


# ============================================================
# 16) Display Results — อ่านจาก session_state เสมอ
#     ผลลัพธ์ไม่หายเมื่อ user scroll / click / interact
# ============================================================
...
if st.session_state.get("result_ready"):
    ...
    force_year = st.session_state["force_year"]
    ...
    with col3:
        review_count = int(
            check_df["balance_check"].astype(str).str.contains(
                "REVIEW|BROKEN|MISSING|NO_PREV", regex=True, na=False
            ).sum()
        )
